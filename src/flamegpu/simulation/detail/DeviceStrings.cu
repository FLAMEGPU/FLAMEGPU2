...
const char* DeviceStrings::getDeviceString(const std::string &host_string) {
    ...
    if (!device_buffer || device_buffer_len < host_buffer_len) {
        ...
        flamegpu::detail::gpuCheck(cudaFree(device_buffer));
        flamegpu::detail::gpuCheck(cudaMalloc(&device_buffer, device_buffer_len));
        device_buffer_occupied = 0;
    }
    // Update device buffer if necessary
    if (device_buffer_occupied < host_buffer_len) {
        flamegpu::detail::gpuCheck(cudaMemcpy(device_buffer, host_buffer.c_str(), host_buffer_len, cudaMemcpyHostToDevice));
