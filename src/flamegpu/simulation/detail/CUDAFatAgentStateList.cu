...
void CUDAFatAgentStateList::resize(const unsigned int minSize, const bool retainData, const flamegpu::detail::gpu::Stream_t stream) {
    ...
    for (auto &buff : variables_unique) {
        ...
        if (retainData && buff->data) {
            ...
            flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemcpyAsync(buff->data_swap, buff->data, active_len, flamegpu::detail::gpu::gpuMemcpyDeviceToDevice, stream));
            ...
            // Zero remaining new data (This will be overwritten before use, so redundant)
            // flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemsetAsync(buff->data_swap, 0, buff_size, stream));
        }
    }
    if (retainData) {
