...
void MessageArray2D::CUDAModelHandler::init(detail::CUDAScatter &scatter, unsigned int streamId, flamegpu::detail::gpu::Stream_t stream) {
    ...
    flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemsetAsync(write_list.at(var.first), 0, var.second.type_size * var.second.elements * hd_metadata.length));
    flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemsetAsync(read_list.at(var.first), 0, var.second.type_size * var.second.elements * hd_metadata.length));
    }
