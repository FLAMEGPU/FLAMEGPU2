...
void HostEnvironment::importMacroProperty(const std::string& property_name, const std::string& file_path) const {
    ...
    try {
        ...
        flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemcpyAsync(m_prop->second.d_ptr, l_prop->second.data(), l_prop->second.size(), flamegpu::detail::gpu::gpuMemcpyHostToDevice, stream));
        ...
        if (extension == ".bin") {
            ...
            // Update the property
            flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemcpyAsync(m_prop->second.d_ptr, buffer.data(), buffer.size(), flamegpu::detail::gpu::gpuMemcpyHostToDevice, stream));
        } else {
            throw;
        }
    }
...
void HostEnvironment::exportMacroProperty(const std::string& property_name, const std::string& file_path, bool pretty_print) const {
    ...
    try {
        ...
        if (extension == ".bin") {
            ...
            flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemcpyAsync(buffer.data(), m_prop->second.d_ptr, m_prop_elements * m_prop->second.type_size, flamegpu::detail::gpu::gpuMemcpyDeviceToHost, stream));
            flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuStreamSynchronize(stream));
            // Output to file
