...
void DeviceExceptionManager::checkError(const std::string &function, const unsigned int streamId, const flamegpu::detail::gpu::Stream_t stream) {
    ...
    if (d_buffer[streamId]) {
        ...
        flamegpu::detail::gpuCheck(flamegpu::detail::gpu::gpuMemcpyAsync(&hd_buffer[streamId], d_buffer[streamId], sizeof(DeviceExceptionBuffer), flamegpu::detail::gpu::gpuMemcpyDeviceToHost, stream));
