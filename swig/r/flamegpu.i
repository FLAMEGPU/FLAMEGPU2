...
%include <std/std_unordered_map.i> // Swig R does not have native version of this include
%include <std/std_array.i> // Swig R does not have native version of this include
%include <std_list.i>
%include <stdint.i>

// argc/argv support
//%include <argcargv.i> // Swig R lacks argcargv equivalent 
...
// TEMPLATE_VARIABLE_INSTANTIATE_FLOATS(uniform, flamegpu::HostRandom::uniformNoRange) // SWIGError: Template 'uniformNoRange' undefined.
// TEMPLATE_VARIABLE_INSTANTIATE_INTS(uniform, flamegpu::HostRandom::uniformRange) // SWIGError: Template 'uniformRange' undefined.
...
// TEMPLATE_SUM_INSTANTIATE(flamegpu::HostAgentAPI) // Swig error: Template 'sumOutT' undefined.
...
// %extend flamegpu::DeviceAgentVector_impl {  // SWIG R reports a syntax error at this line?
    // %pythoncode {
        // def __iter__(self):
            // return FLAMEGPUIterator(self)
        // def __len__(self):
...
//%template(LogFrameList) std::list<flamegpu::LogFrame>; //Can't get SWIG to be happy with these, has scope issues putting them in scope of last %include, rather than std
...
#undef VISUALISATION // Temp disable bc includes below are causing problems
#ifdef VISUALISATION
