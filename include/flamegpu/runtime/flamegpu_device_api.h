...
class FLAMEGPU_DEVICE_API {
    ...
    friend __global__ void agent_function_wrapper(Curve::CurveNamespaceHash, Curve::CurveNamespaceHash, Curve::CurveNamespaceHash, FLAMEGPU_AGENT_FUNCTION_POINTER, const int, const unsigned int, const unsigned int);
 ...
 public:
    ...
    */
     ...
     * \param messagename_hash
     */
    __device__ void setMessageOutpNameSpace(Curve::CurveNamespaceHash messagename_hash) {
...
__device__ T FLAMEGPU_DEVICE_API::getVariable(const char(&variable_name)[N]) {
    ...
    T value = Curve::curveGetVariable<T>(variable_name, agent_func_name_hash , index);
...
__device__ void FLAMEGPU_DEVICE_API::setVariable(const char(&variable_name)[N], T value) {
    ...
    Curve::curveSetVariable<T>(variable_name , agent_func_name_hash,  value, index);
}
...
__device__ T FLAMEGPU_DEVICE_API::getMessageVariable(const char(&variable_name)[N]) {
    ...
    // get the value from curve
    T value = Curve::curveGetVariable<T>(variable_name, agent_func_name_hash + messagename_inp_hash, index);
...
__device__ void FLAMEGPU_DEVICE_API::setMessageVariable(const char(&variable_name)[N], T value) {
    ...
    // set the variable using curve
    Curve::curveSetVariable<T>(variable_name, agent_func_name_hash + messagename_outp_hash, value, index);
}
