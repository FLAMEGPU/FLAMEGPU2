...
class MessageArray2D::In {
 public:
    ...
    /**
     * This class is created when a search origin is provided to MessageArray2D::In::wrap(size_type, size_type, size_type = 1)
    ...
    class WrapFilter {
     ...
     public:
         ...
         */
        inline __device__ WrapFilter(const MetaData *_metadata, const size_type &x, const size_type &y, const size_type &_radius);
#if !defined(SEATBELTS) || SEATBELTS
        /**
    ...
    class Filter {
     ...
     public:
         ...
         * @param _radius Search radius
         */
        inline __device__ Filter(const MetaData *_metadata, const size_type &x, const size_type &y, const size_type &_radius);
#if !defined(SEATBELTS) || SEATBELTS
        /**
         * A null filter which always returns 0 messages
         */
    ...
    /**
     * This class is created when a search origin is provided to MessageArray2D::In::vn_wrap(size_type, size_type, size_type = 1)
     * It provides iterator access to a subset of the full message list, according to the provided search origin and radius
     * The radius wraps the message list bounds
     *
     * @see MessageArray2D::In::vn_wrap(size_type, size_type, size_type)
     */
    class VonNeumannWrapFilter {
     ...
     public:
         ...
         * @param _radius Search radius
         */
        inline __device__ VonNeumannWrapFilter(const MetaData *_metadata, const size_type &x, const size_type &y, const size_type &_radius);
#if !defined(SEATBELTS) || SEATBELTS
        /**
         * A null filter which always returns 0 messages
         */
    ...
    /**
     * This class is created when a search origin is provided to MessageArray2D::In::vn(size_type, size_type, size_type = 1)
     * It provides iterator access to a subset of the full message list, according to the provided search origin and radius
     * The radius does not wrap the message list bounds
     *
     * @see MessageArray2D::In::vn(size_type, size_type, size_type)
     */
    class VonNeumannFilter {
     ...
     public:
         ...
         * @param _radius Search radius
         */
        inline __device__ VonNeumannFilter(const MetaData *_metadata, const size_type &x, const size_type &y, const size_type &_radius);
#if !defined(SEATBELTS) || SEATBELTS
        /**
         * A null filter which always returns 0 messages
         */
    ...
    { }
    /**
     * Returns a Filter object which provides access to the wrapped Moore neighbourhood message iterator
    ...
    /**
     * Returns a Filter object which provides access to the Moore neighbourhood message iterator
     * for iterating a subset of messages including those within the radius of the search origin
     * this excludes the message at the search origin
     *
     * @param x Search origin x coord
    ...
    inline __device__ Filter operator() (const size_type &x, const size_type &y, const size_type &radius = 1) const {
     ...
     /**
     * Returns a Filter object which provides access the wrapped Von Neumann neighbourhood message iterator
     * for iterating a subset of messages including those within the radius of the search origin
     * this excludes the message at the search origin
     * The radius will wrap over environment bounds
     *
...
#endif
        return VonNeumannWrapFilter(metadata, x, y, radius);
    }
    /**
     * Returns a Filter object which provides access to the Von Neumann neighbourhood message iterator
     * for iterating a subset of messages including those within the radius of the search origin
     * this excludes the message at the search origin
     *
     * @param x Search origin x coord
     ...
     * @note The location [x, y] must be within the bounds of the message list
     */
    inline __device__ VonNeumannFilter vn(const size_type &x, const size_type &y, const size_type &radius = 1) const {
#if !defined(SEATBELTS) || SEATBELTS
        if (radius == 0) {
            DTHROW("%u is not a valid radius for accessing Array2D message lists.\n", radius);
...
#endif
        return VonNeumannFilter(metadata, x, y, radius);
    }
    /**
...
template<typename T, unsigned int N>
__device__ T MessageArray2D::In::VonNeumannWrapFilter::Message::getVariable(const char(&variable_name)[N]) const {
#if !defined(SEATBELTS) || SEATBELTS
    // Ensure that the message is within bounds.
    if (index_1d >= this->_parent.metadata->length) {
        DTHROW("Invalid Array2D message, unable to get variable '%s'.\n", variable_name);
...
template<typename T, unsigned int N>
__device__ T MessageArray2D::In::VonNeumannFilter::Message::getVariable(const char(&variable_name)[N]) const {
#if !defined(SEATBELTS) || SEATBELTS
    // Ensure that the message is within bounds.
    if (index_1d >= this->_parent.metadata->length) {
        DTHROW("Invalid Array2D message, unable to get variable '%s'.\n", variable_name);
...
// Moore
__device__ MessageArray2D::In::Filter::Filter(const MetaData *_metadata,  const size_type &x, const size_type &y, const size_type &_radius)
    : metadata(_metadata) {
    loc[0] = x;
    loc[1] = y;
...
// Von Neumann Wrap
__device__ MessageArray2D::In::VonNeumannWrapFilter::VonNeumannWrapFilter(const MetaData* _metadata, const size_type& x, const size_type& y, const size_type& _radius)
...
#if !defined(SEATBELTS) || SEATBELTS
__device__ inline MessageArray2D::In::VonNeumannWrapFilter::VonNeumannWrapFilter()
    : radius(0)
    , metadata(nullptr) {
    loc[0] = 0;
    loc[1] = 0;
...
#endif
__device__ MessageArray2D::In::VonNeumannWrapFilter::Message& MessageArray2D::In::VonNeumannWrapFilter::Message::operator++() {
#if !defined(SEATBELTS) || SEATBELTS
    if (!_parent.metadata)
        return *this;
#endif
    ...
    } while (abs(relative_cell[0]) + abs(relative_cell[1]) > static_cast<int>(_parent.radius));
    // Wrap over boundaries
    const unsigned int their_x = (this->_parent.loc[0] + relative_cell[0] + this->_parent.metadata->dimensions[0]) % this->_parent.metadata->dimensions[0];
    const unsigned int their_y = (this->_parent.loc[1] + relative_cell[1] + this->_parent.metadata->dimensions[1]) % this->_parent.metadata->dimensions[1];
    // Solve to 1 dimensional bin index
...
// Von Neumann
__device__ MessageArray2D::In::VonNeumannFilter::VonNeumannFilter(const MetaData* _metadata, const size_type& x, const size_type& y, const size_type& _radius)
    : radius(static_cast<int>(_radius))
    , metadata(_metadata) {
    loc[0] = x;
    loc[1] = y;
...
#if !defined(SEATBELTS) || SEATBELTS
__device__ inline MessageArray2D::In::VonNeumannFilter::VonNeumannFilter()
    : radius(0)
    , metadata(nullptr) {
    loc[0] = 0;
    loc[1] = 0;
...
#endif
__device__ MessageArray2D::In::VonNeumannFilter::Message& MessageArray2D::In::VonNeumannFilter::Message::operator++() {
#if !defined(SEATBELTS) || SEATBELTS
    if (!_parent.metadata)
        return *this;
#endif
