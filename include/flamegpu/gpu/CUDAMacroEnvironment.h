...
HostMacroProperty<T, I, J, K, W> CUDAMacroEnvironment::getProperty(const std::string& name) {
    ...
    if (prop == properties.end()) {
        THROW flamegpu::exception::InvalidEnvProperty("Environment property with name '%s' not found, "
            "in HostEnvironment::getMacroProperty()\n",
            name.c_str());
    } else if (prop->second.type != std::type_index(typeid(T))) {
        THROW flamegpu::exception::InvalidEnvProperty("Environment property '%s' type mismatch '%s' != '%s', "
            "in HostEnvironment::getMacroProperty()\n",
        ...
        THROW flamegpu::exception::InvalidEnvProperty("Environment property '%s' dimensions mismatch (%u, %u, %u, %u) != (%u, %u, %u, %u), "
            "in HostEnvironment::getMacroProperty()\n",
    ...
    auto ret = std::make_shared<HostMacroProperty_MetaData>(prop->second.d_ptr, prop->second.elements, sizeof(T));
