...
class MPIEnsemble {  
 ...
 public:
    ...
    MPIEnsemble(const CUDAEnsemble::EnsembleConfig &_config)
    ...
    int recieveErrors(std::multimap<int, AbstractSimRunner::ErrorDetail> &err_detail, const unsigned int total_runs) {
        int errCount = 0;
        if (world_rank == 0) {
            MPI_Status status;
            int flag;
    ...
    int recieveJobRequests(unsigned int &next_run, const unsigned int total_runs) {
