...
// Define a macro for a test which is disbaled for non-CUDA builds
#if defined(FLAMEGPU_USE_CUDA)
#define CUDA_ONLY_TEST(test_suite, test_name) TEST(test_suite, test_name)
#else  // defined(FLAMEGPU_USE_CUDA)
#define CUDA_ONLY_TEST(test_suite, test_name) TEST(test_suite, DISABLED_ ## test_name)
#endif  // defined(FLAMEGPU_USE_CUDA)

// flamegpu::detail::gpu::cuda::compute_capability is CUDA-only, so deine a macro which marks these tests as disabled otherwise.

// Test the getting of a device's compute capability.
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, getComputeCapability) {
#if defined(FLAMEGPU_USE_CUDA)
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        EXPECT_EQ(detail::gpu::cuda::compute_capability::getComputeCapability(i), reference);
    }

    // If the function is given a bad index, it should throw.
    EXPECT_ANY_THROW(detail::gpu::cuda::compute_capability::getComputeCapability(-1));
    EXPECT_ANY_THROW(detail::gpu::cuda::compute_capability::getComputeCapability(device_count));
#endif  // defined(FLAMEGPU_USE_CUDA)
}
/**
...
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, minimumCompiledComputeCapability) {
#if defined(FLAMEGPU_USE_CUDA)
    // If the macro is defined, the returned value should match, otherwise it should be 0.
    const int min_arch = detail::gpu::cuda::compute_capability::minimumCompiledComputeCapability();
...
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, compiledCompiledComputeCapabilitiesString) {
#if defined(FLAMEGPU_USE_CUDA)
    const std::string arch_list_str = detail::gpu::cuda::compute_capability::compiledCompiledComputeCapabilitiesString();
...
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, checkComputeCapability) {
#if defined(FLAMEGPU_USE_CUDA)
    // Get the number of cuda devices
    int device_count = 0;
    if (cudaSuccess != cudaGetDeviceCount(&device_count) || device_count <= 0) {
    ...
    for (int i = 0; i < device_count; i++) {
        ...
        int cc = detail::gpu::cuda::compute_capability::getComputeCapability(i);
        EXPECT_EQ(detail::gpu::cuda::compute_capability::checkComputeCapability(i), cc >= min_cc);
    }

    // If the function is given a bad index, it should throw and the result is irrelevant.
    EXPECT_ANY_THROW(detail::gpu::cuda::compute_capability::checkComputeCapability(-1));
    EXPECT_ANY_THROW(detail::gpu::cuda::compute_capability::checkComputeCapability(device_count));
#endif  // defined(FLAMEGPU_USE_CUDA)
}
 ...
 */
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, getNVRTCSupportedComputeCapabilties) {
#if defined(FLAMEGPU_USE_CUDA)
    std::vector<int> architectures = detail::gpu::cuda::compute_capability::getNVRTCSupportedComputeCapabilties();
 ...
 */
CUDA_ONLY_TEST(TestDetailGPUCudaComputeCapability, selectAppropraiteComputeCapability) {
#if defined(FLAMEGPU_USE_CUDA)
    // Check an exact match should be found
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, {86}), 86);
    // Check a miss but with a lower value returns the lower value
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, {80}), 80);
    // Check a miss without a valid value returns 0
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, {90}), 0);
    // Check a miss occurs when no values are present in the vector.
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, {}), 0);
    ...
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, CUDA_11_1_ARCHES), 86);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(60, CUDA_11_1_ARCHES), 60);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(30, CUDA_11_1_ARCHES), 0);
    ...
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, CUDA_11_0_ARCHES), 80);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(60, CUDA_11_0_ARCHES), 60);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(30, CUDA_11_0_ARCHES), 0);
    ...
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(86, CUDA_10_0_ARCHES), 75);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(60, CUDA_10_0_ARCHES), 60);
    EXPECT_EQ(detail::gpu::cuda::compute_capability::selectAppropraiteComputeCapability(30, CUDA_10_0_ARCHES), 30);
#endif  // defined(FLAMEGPU_USE_CUDA)
}
