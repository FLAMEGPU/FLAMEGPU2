...
FLAMEGPU_STEP_FUNCTION(VALIDATE_ENV) {
    EXPECT_EQ(FLAMEGPU->environment.get<float>("float"), 12.0f);
    EXPECT_EQ(FLAMEGPU->environment.get<float>("float"), 12.0f);
    EXPECT_EQ(FLAMEGPU->environment.get<double>("double"), 13.0);
    EXPECT_EQ(FLAMEGPU->environment.get<int64_t>("int64_t"), 14);
    EXPECT_EQ(FLAMEGPU->environment.get<uint64_t>("uint64_t"), 15u);
    EXPECT_EQ(FLAMEGPU->environment.get<int32_t>("int32_t"), 16);
    EXPECT_EQ(FLAMEGPU->environment.get<uint32_t>("uint32_t"), 17u);
    EXPECT_EQ(FLAMEGPU->environment.get<int16_t>("int16_t"), 18);
    EXPECT_EQ(FLAMEGPU->environment.get<uint16_t>("uint16_t"), 19u);
    EXPECT_EQ(FLAMEGPU->environment.get<int8_t>("int8_t"), 20);
    EXPECT_EQ(FLAMEGPU->environment.get<uint8_t>("uint8_t"), 21u);
    const bool float_array_eq = FLAMEGPU->environment.get<float, 3>("float_a") == std::array<float, 3>{ 12.0f, 0.0f, 1.0f };
    EXPECT_TRUE(float_array_eq);
    const bool double_array_eq = FLAMEGPU->environment.get<double, 3>("double_a") == std::array<double, 3>{ 13.0, 0.0, 1.0 };
    EXPECT_TRUE(double_array_eq);
    const bool int64_t_array_eq = FLAMEGPU->environment.get<int64_t, 3>("int64_t_a") == std::array<int64_t, 3>{ 14, 0, 1 };
    EXPECT_TRUE(int64_t_array_eq);
    const bool uint64_t_array_eq = FLAMEGPU->environment.get<uint64_t, 3>("uint64_t_a") == std::array<uint64_t, 3>{ 15u, 0u, 1u };
    EXPECT_TRUE(uint64_t_array_eq);
    const bool int32_t_array_eq = FLAMEGPU->environment.get<int32_t, 3>("int32_t_a") == std::array<int32_t, 3>{ 16, 0, 1 };
    EXPECT_TRUE(int32_t_array_eq);
    const bool uint32_t_array_eq = FLAMEGPU->environment.get<uint32_t, 3>("uint32_t_a") == std::array<uint32_t, 3> { 17u, 0u, 1u };
    EXPECT_TRUE(uint32_t_array_eq);
    const bool int16_t_array_eq = FLAMEGPU->environment.get<int16_t, 3>("int16_t_a") == std::array<int16_t, 3>{ 18, 0, 1 };
    EXPECT_TRUE(int16_t_array_eq);
    const bool uint16_t_array_eq = FLAMEGPU->environment.get<uint16_t, 3>("uint16_t_a") == std::array<uint16_t, 3>{ 19u, 0u, 1u };
    EXPECT_TRUE(uint16_t_array_eq);
    const bool int8_t_array_eq = FLAMEGPU->environment.get<int8_t, 3>("int8_t_a") == std::array<int8_t, 3>{ 20, 0, 1 };
    EXPECT_TRUE(int8_t_array_eq);
    const bool uint8_t_array_eq = FLAMEGPU->environment.get<uint8_t, 3>("uint8_t_a") == std::array<uint8_t, 3>{ 21u, 0u, 1u };
    EXPECT_TRUE(uint8_t_array_eq);
    validate_has_run = true;
}
FLAMEGPU_STEP_FUNCTION(RESET_ENV) {
    FLAMEGPU->environment.set<float>("float", {});
    FLAMEGPU->environment.set<double>("double", {});
    FLAMEGPU->environment.set<int64_t>("int64_t", {});
    FLAMEGPU->environment.set<uint64_t>("uint64_t", {});
    FLAMEGPU->environment.set<int32_t>("int32_t", {});
    FLAMEGPU->environment.set<uint32_t>("uint32_t", {});
    FLAMEGPU->environment.set<int16_t>("int16_t", {});
    FLAMEGPU->environment.set<uint16_t>("uint16_t", {});
    FLAMEGPU->environment.set<int8_t>("int8_t", {});
    FLAMEGPU->environment.set<uint8_t>("uint8_t", {});
    FLAMEGPU->environment.set<float, 3>("float_a", {});
    FLAMEGPU->environment.set<double, 3>("double_a", {});
    FLAMEGPU->environment.set<int64_t, 3>("int64_t_a", {});
    FLAMEGPU->environment.set<uint64_t, 3>("uint64_t_a", {});
    FLAMEGPU->environment.set<int32_t, 3>("int32_t_a", {});
    FLAMEGPU->environment.set<uint32_t, 3>("uint32_t_a", {});
    FLAMEGPU->environment.set<int16_t, 3>("int16_t_a", {});
    FLAMEGPU->environment.set<uint16_t, 3>("uint16_t_a", {});
    FLAMEGPU->environment.set<int8_t, 3>("int8_t_a", {});
    FLAMEGPU->environment.set<uint8_t, 3>("uint8_t_a", {});
}
TEST(TestXML_IO, WriteRead) {
    ...
    }
    {
        ...
        e.add<float>("float", 12.0f);
        e.add<double>("double", 13.0);
        e.add<int64_t>("int64_t", 14);
        e.add<uint64_t>("uint64_t", 15u);
        e.add<int32_t>("int32_t", 16);
        e.add<uint32_t>("uint32_t", 17u);
        e.add<int16_t>("int16_t", 18);
        e.add<uint16_t>("uint16_t", 19u);
        e.add<int8_t>("int8_t", 20);
        e.add<uint8_t>("uint8_t", 21u);
        e.add<float, 3>("float_a", { 12.0f, 0.0f, 1.0f });
        e.add<double, 3>("double_a", { 13.0, 0.0, 1.0 });
        e.add<int64_t, 3>("int64_t_a", { 14, 0, 1 });
        e.add<uint64_t, 3>("uint64_t_a", { 15u, 0u, 1u });
        e.add<int32_t, 3>("int32_t_a", { 16, 0, 1 });
        e.add<uint32_t, 3>("uint32_t_a", { 17u, 0u, 1u });
        e.add<int16_t, 3>("int16_t_a", { 18, 0, 1});
        e.add<uint16_t, 3>("uint16_t_a", { 19u, 0u, 1u });
        e.add<int8_t, 3>("int8_t_a", { 20, 0, 1 });
        e.add<uint8_t, 3>("uint8_t_a", {21u, 0u, 1u});
    }
