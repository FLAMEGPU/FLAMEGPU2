...
class DeviceAgentCreationTest(TestCase):
    ...
    def test_device_agent_birth_array_unsuitable(self): 
        # Curve will error, however agent creation will go through, agent will have default value
        TEST_REFERENCE =  (3, 5, 9, 17)
        model = pyflamegpu.ModelDescription("test_device_agent_birth_array_unsuitable")
        agent = model.newAgent("agent_name")
        agent.newVariableIntArray4("array_var", TEST_REFERENCE)
        fn = agent.newRTCFunction("out", self.ArrayVarDeviceBirth_ArrayUnsuitable)
        fn.setAllowAgentDeath(True)
        fn.setAgentOutput(agent)
        model.newLayer().addAgentFunction(fn)
        # Run the init function
