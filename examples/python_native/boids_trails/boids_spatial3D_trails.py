...
def inputdata(message_in: pyflamegpu.MessageSpatial3D, message_out: pyflamegpu.MessageNone):
    ...
    wing_animation = math.sinf(wing_position)
    pyflamegpu.setVariableFloat("wing_position", wing_position)
    pyflamegpu.setVariableFloat("wing_animation", wing_animation)

    # create tail
    pyflamegpu.agent_out.setVariableFloat("x", agent_x)
    pyflamegpu.agent_out.setVariableFloat("y", agent_y)
    pyflamegpu.agent_out.setVariableFloat("z", agent_z)
    pyflamegpu.agent_out.setVariableFloat("fx", agent_fx)
    pyflamegpu.agent_out.setVariableFloat("fy", agent_fy)
    pyflamegpu.agent_out.setVariableFloat("fz", agent_fz)
    pyflamegpu.agent_out.setVariableFloat("colour", 0)
    pyflamegpu.agent_out.setVariableUInt("alive_steps", 0)

    return pyflamegpu.ALIVE


@pyflamegpu.agent_function
def process_tail(message_in: pyflamegpu.MessageNone, message_out: pyflamegpu.MessageNone):
    """
    Pure python version of output agent function for Tail agent. Checks to see if it has been alive long enough to die.
    """
    alive_steps = pyflamegpu.getVariableUInt("alive_steps")
    TAILS_NUMBER_PER_AGENT = pyflamegpu.environment.getPropertyUInt("TAILS_NUMBER_PER_AGENT")

    # die f max number of steps has been exceeded
    if alive_steps >= TAILS_NUMBER_PER_AGENT:
        return pyflamegpu.DEAD

    # increment the number of steps and return alive
    alive_steps+=1
    pyflamegpu.setVariableUInt("alive_steps", alive_steps)
    pyflamegpu.setVariableFloat("colour", float(alive_steps)/float(TAILS_NUMBER_PER_AGENT))
    return pyflamegpu.ALIVE
...
# Tails
env.newPropertyUInt("TAILS_NUMBER_PER_AGENT", 124)
...
# Tail agent
tail_agent = model.newAgent("Tail")
tail_agent.newVariableFloat("x")
tail_agent.newVariableFloat("y")
tail_agent.newVariableFloat("z")
tail_agent.newVariableFloat("fx")
tail_agent.newVariableFloat("fy")
tail_agent.newVariableFloat("fz")
tail_agent.newVariableFloat("colour")
tail_agent.newVariableUInt("alive_steps")
process_tail_translated = pyflamegpu.codegen.translate(process_tail)
tail_agent_fn = tail_agent.newRTCFunction("process_tail", process_tail_translated)
tail_agent_fn.setAllowAgentDeath(True)
...
inputdata_fn = agent.newRTCFunction("inputdata", inputdata_translated)
inputdata_fn.setMessageInput("location")
inputdata_fn.setAgentOutput(tail_agent)
...
# Layer #3
model.newLayer().addAgentFunction("Tail", "process_tail")
...
if pyflamegpu.VISUALISATION:
    ...
    # Tail agent
    tail_agt = visualisation.addAgent("Tail")
    tail_agt.setForwardXVariable("fx")
    tail_agt.setForwardYVariable("fy")
    tail_agt.setForwardZVariable("fz")
    tail_agt.setModel(pyflamegpu.ARROWHEAD) # Alternative simple 3D model for very large pop sizes
    #tail_agt.setColor(pyflamegpu.BLACK);
    tail_agt.setColor(pyflamegpu.HSVInterpolation("colour", 0.0, 360.0))
    tail_agt.setModelScale(env.getPropertyFloat("SEPARATION_RADIUS") /2.0)
