...
X_VARIABLE_PLACEHOLDER = "$X"
X_VARIABLE_PLACEHOLDER = "$Y"
RESOURCE_VARIABLE_PLACEHOLDER = "$R"
STATUS_VARIABLE_PLACEHOLDER = "$S"
LOC_ID_VARIABLE_PLACEHOLDER = "$LOC"
AGENT_ID_VARIABLE_PLACEHOLDER = "$ID"

STATUS_UNOCCUPIED_VALUE = 0
STATUS_OCCUPIED_VALUE = 1
STATUS_MOVEMENT_REQUESTED_VALUE  = 2
STATUS_MOVEMENT_UNRESOLVED_VALUE  = 3


output_cell_status_str = """
...
def output_cell_status(message_in: pyflamegpu.MessageNone, message_out: pyflamegpu.MessageArray2D):
    ...
    message_out.setVariableInt("$LOC", pyflamegpu.getID())
    message_out.setVariableInt("$S", pyflamegpu.getVariableInt("$S"))
    message_out.setVariableInt("$R", pyflamegpu.getVariableInt("$R"))

    return pyflamegpu.ALIVE
"""

movement_request_str = """
@pyflamegpu.agent_function
def movement_request(message_in: pyflamegpu.MessageArray2D, message_out: pyflamegpu.MessageArray2D):
    best_R_level = -1
    best_R_lrandom = float(-1)
    best_location_id = 0
    ...
    status = pyflamegpu.getVariableInt("$S")

    agent_x = pyflamegpu.getVariableInt("$X")
    agent_y = pyflamegpu.getVariableInt("$Y")

    # if occupied then look for empty cells
    if (status == $STATUS_MOVEMENT_UNRESOLVED_VALUE) :
        for current_message in message_in.wrap(agent_x, agent_y) :
            # if location is unoccupied then check for empty locations
            if (current_message.getVariableInt("$S") == $STATUS_UNOCCUPIED_VALUE) :
                # if the sugar level at current location is better than currently stored then update
                message_R = current_message.getVariableInt("$R")
                message_priority = pyflamegpu.random.uniformFloat()
                if ((message_R >  best_R_level)   or
                    (message_R == best_R_level    and 
                     message_priority        >  best_R_lrandom)) :
                    best_R_level = message_R
                    best_R_lrandom = message_priority
                    best_location_id = current_message.getVariableInt("$LOC")
        ...
        if best_location_id != 0 :
            status = $STATUS_MOVEMENT_REQUESTED_VALUE
        else : 
            status = $STATUS_OCCUPIED_VALUE          
        pyflamegpu.setVariableInt("£R", status)

    # add a movement request
    message_out.setIndex(agent_x, agent_y)
    message_out.setVariableInt("$ID", pyflamegpu.getVariableInt("$ID"))
    message_out.setVariableInt("$LOC", best_location_id)
    message_out.setVariableInt("R_level", pyflamegpu.getVariableInt("sugar_level"))
    message_out.setVariableInt("metabolism", pyflamegpu.getVariableInt("metabolism"))

    return pyflamegpu.ALIVE
"""

movement_response_str_ = """
@pyflamegpu.agent_function
    ...
    for current_message in message_in.wrap(agent_x, agent_y) :
        ...
        if (status == pyflamegpu.environment.getPropertyInt("agent_status_unoccupied")) :
            ...
            if (current_message.getVariableInt("location_id") == location_id) :
                ...
                message_priority = pyflamegpu.random.uniformFloat()
                if (status == pyflamegpu.environment.getPropertyInt("agent_status_unoccupied")) and (message_priority > best_request_priority) :
                    pyflamegpu.setVariableInt("status", pyflamegpu.environment.getPropertyInt("agent_status_occupied"))
                    pyflamegpu.setVariableInt("agent_id", current_message.getVariableInt("agent_id"))
                    best_request_priority = message_priority
                    for var_name in AGENT_VARIABLES:
                        pyflamegpu.setVariableInt("{var_name}" = current_message.getVariableInt("{var_name}")
